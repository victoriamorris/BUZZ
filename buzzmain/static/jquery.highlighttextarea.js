...
(function($){
    ...
    Highlighter.prototype.highlight = function() {
        ...
        var colourMap = {};
        var wordList = [];
        $.each(this.settings.words, function (colour, words) {
            that.colourMatches[colour] = 0;
            colourMap[colour] = words.map(function (word) {
                return '(' + word + ')';
                }).join('|');
        ...
        for (const [colour, words] of Object.entries(colourMap)) {
            var regex = new RegExp(colourMap[colour], 'gum');

            var wordMatches = text.match(regex);
            if (wordMatches) {
                text = text.replace(regex, function(innerMatch, start, contents) {
                matches.push(innerMatch);
                that.colourMatches[colour] += 1;
                return '<span class="ul_'+ colour +'">' + innerMatch + '</span>';
              });
            }
        }
    ...
    Highlighter.prototype.destroy = function() {
        ...
        Utilities.cloneCss(this.$container, this.$el, [
            'background-image','background-colour','background-position','background-repeat',
    ...
    Highlighter.prototype.setOptions = function(options) {
        ...
        if (!$.isEmptyObject(this.settings.words)) {
            this.settings.words = Utilities.cleanWords(this.settings.words, this.settings.colour);
            this.settings.ranges = {};
        }
        else if (!$.isEmptyObject(this.settings.ranges)) {
            this.settings.words = {};
            this.settings.ranges = Utilities.cleanRanges(this.settings.ranges, this.settings.colour);
        }
    ...
    /*
     * Formats a list of words into a hash of arrays (Colour => Words list)
     * @param words {mixed}
     * @param colour {string} default colour
     * @return {object[]}
     */
    Utilities.cleanWords = function(words, colour) {
        var out = {};
        ...
        for (var i=0, l=words.length; i<l; i++) {
            ...
            if ($.isPlainObject(group)) {
                ...
                for (var j=0, m=group.words.length; j<m; j++) {
                    out[group.colour].push(Utilities.htmlEntities(group.words[j]));
                }
            }
            else {
                if (!out[colour]) {
                ...
                out[colour].push(Utilities.htmlEntities(group));
            }
        }
    ...
    /*
     * Formats a list of ranges into a hash of arrays (Colour => Ranges list)
     * @param ranges {mixed}
     * @param colour {string} default colour
     * @return {object[]}
     */
    Utilities.cleanRanges = function(ranges, colour) {
        var out = [];
